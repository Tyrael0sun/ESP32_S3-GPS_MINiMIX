...
## 3\. 技术架构
  ...
  * **UI:** `LVGL v8.3` (图形库) + `ESP LCD API` (ST7789显示驱动)
  * **核心组件:**
      * **显示:** 通过高速SPI3驱动 `ST7789` LCD (240×320分辨率，竖屏模式，旋转180°)，双缓冲DMA传输。
      * **存储:** 通过4-bit SDIO模式高速读写SD卡。
      * **输入:** 
        - 旋转编码器: PCNT硬件脉冲计数，3-step阈值过滤，500ms自动清零防抖
        - 主功能按键: GPIO中断+消抖，支持短按/中按/长按/双击
  * **代码结构:**
  ...
  * **固件规格:**
      * 二进制大小: 660.8KB (0xa1350 bytes)
      * 分区剩余: 37% (0x5ecb0 bytes)
      * Flash命令: `idf.py flash`
...
#### 用户输入 (INPUT)
  ...
  * **F-INPUT-02:** 在主功能界面之间切换的操作定义为：旋转编码器切换模式（自行车码表→GPS轨迹记录→P-Box→GNSS信息→设置→自行车码表），短按执行确认操作，中按启动/停止轨迹记录，长按进入/退出设置界面。
  * **F-INPUT-03:** 按键事件的时间定义为：消抖时间50ms，短按持续时间约50ms，中按持续时间约500ms，长按持续时间约2000ms。
  * **F-INPUT-04:** 旋转编码器应使用3-step阈值过滤（±3 steps = ±1输出），并在500ms无变化时自动清零，防止机械振动导致的计数漂移。
  * **F-INPUT-05:** 旋转编码器GPIO引脚（A相/B相）应配置为输入上拉模式，确保信号稳定性。
...
#### 诊断与日志 (DIAG)
  ...
  * **F-DIAG-01:** 设备启动后的前5秒内，应以高频（1秒一次）输出硬件自检状态到串口。
  * **F-DIAG-02:** 5秒后切换为低频心跳模式（5秒一次），持续监控系统状态。
  * **F-DIAG-03:** 自检报告必须包含以下模块的状态和关键读数：GNSS模块、IMU（线性加速度和重力分量）、磁力计、气压计，以及四个来源的温度读数（MCU内部/IMU/气压计/磁力计）。
  * **F-DIAG-04:** 当旋转编码器转动或按键按下时，应通过diagnostics_trigger()函数立即上报事件和按键持续时间。
  * **F-DIAG-05:** GPS模块应支持波特率自动切换：上电默认9600bps，通过PMTK251命令切换至115200bps。
